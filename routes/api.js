...
const nekohime = require("../lib/listdl")
...
const { doujindesu, facebook, instagram, h5tuqq, bokepsin  } = require("@xct007/frieren-scraper")
const router = express.Router()
let fs = require("fs")
let fetch = require('node-fetch')
let { Configuration, OpenAIApi } = require("openai")
const configuration = new Configuration({
    apiKey: "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"
});
...
async function cekPrem(req, res, next) {
	...
	if (!db.premium) {
		return res.json({ status : false, creator : `${creator}`, message : "[!] Apikeymu Tidak Premium"})  
	}else{
        return next();
    }
}
...
async function cekKey(req, res, next) {
    ...
    if(db === null) {
		return res.json({ status : false, creator : `${creator}`, message : "[!] Apikey Tidak Ada"})  
				...
				return res.json({ status : false, creator : `${creator}`, message : "[!] Silakan Verify Email dulu sebelum gunakan apikey"})  
...
async function limitapikey(apikey) {
	   ...
	   await dataweb.updateOne({}, {$inc: {  TotalRequest: 1 }})
...
nekohime.twitter(url).then(data => {
...
router.get('/api/dowloader/igdowloader', cekKey, async (req, res, next) => {
	...
	instagram.v1(url).then(async (data) => {
...
router.get('/api/dowloader/xnxx', cekKey, cekPrem, async (req, res, next) => {
	var url = req.query.url
	if (!url ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter url"})   

	xnxxdl(url).then(async (data) => {
		if (!data ) return res.json(loghandler.error) 
		limitapikey(req.query.apikey)
		res.json({
			status: true,
...
router.get('/api/dowloader/doujindesu', cekKey, cekPrem, async (req, res, next) => {
	var url = req.query.url
	if (!url ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter url"})   

	doujindesu.detail(url).then(async (data) => {
		if (!data ) return res.json(loghandler.error) 
		limitapikey(req.query.apikey)
		res.json({
			status: true,
...
router.get('/api/dowloader/soundcloud', cekKey, async (req, res, next) => {
	...
	nekohime.soundcloud(url).then(data => {
...
router.get('/api/dowloader/mediafire', cekKey, async (req, res, next) => {
	...
	nekohime.mediafiredl(url).then(async (data) => {
...
router.get('/api/dowloader/sfilemobi', cekKey, async (req, res, next) => {
	...
	nekohime.sfilemobi(url).then(async (data) => {
		if (!data ) return res.json(loghandler.noturl)
		limitapikey(req.query.apikey)
		res.json({
			status: true,
...
router.get('/api/dowloader/zippyshare', cekKey, async (req, res, next) => {
	...
	nekohime.zippyshare(url).then(async (data) => {
		if (!data ) return res.json(loghandler.noturl)
		limitapikey(req.query.apikey)
		res.json({
			status: true,
...
router.get('/api/dowloader/telesticker', cekKey, async (req, res, next) => {
	...
	nekohime.telesticker(url).then(data => {
		limitapikey(req.query.apikey)
		res.json({
			status: true,
	        creator: `${creator}`,
...
router.get('/api/textpro/pencil', cekKey, async (req, res, next) => {
	...
	nekohime.textpro("https://textpro.me/create-a-sketch-text-effect-online-1044.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
...
router.get('/api/textpro/glitch', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.textpro("https://textpro.me/create-impressive-glitch-text-effects-online-1027.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/textpro/blackpink', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.textpro("https://textpro.me/create-blackpink-logo-style-online-1001.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/textpro/berry', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.textpro("https://textpro.me/create-berry-text-effect-online-free-1033.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/textpro/neon', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.textpro("https://textpro.me/neon-light-text-effect-online-882.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/textpro/logobear', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.textpro("https://textpro.me/online-black-and-white-bear-mascot-logo-creation-1012.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/textpro/3dchristmas', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.textpro("https://textpro.me/3d-christmas-text-effect-by-name-1055.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/textpro/thunder', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.textpro("https://textpro.me/online-thunder-text-effect-generator-1031.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/textpro/3dboxtext', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.textpro("https://textpro.me/3d-box-text-effect-online-880.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/textpro/glitch2', cekKey, async (req, res, next) => {
	...
	nekohime.textpro("https://textpro.me/create-a-glitch-text-effect-online-free-1026.html", [text1,text2])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/textpro/glitchtiktok', cekKey, async (req, res, next) => {
	...
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	if (!text2 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text2"}) 
	nekohime.textpro("https://textpro.me/create-glitch-text-effect-style-tik-tok-983.html", [text1,text2])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/textpro/video-game-classic', cekKey, async (req, res, next) => {
	...
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	if (!text2 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text2"}) 
	nekohime.textpro("https://textpro.me/video-game-classic-8-bit-text-effect-1037.html", [text1,text2])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/textpro/marvel-studios', cekKey, async (req, res, next) => {
	...
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	if (!text2 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text2"}) 
	nekohime.textpro("https://textpro.me/create-logo-style-marvel-studios-online-971.html", [text1,text2])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/textpro/ninja-logo', cekKey, async (req, res, next) => {
	...
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	if (!text2 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text2"}) 
	nekohime.textpro("https://textpro.me/create-ninja-logo-online-935.html", [text1,text2])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/textpro/green-horror', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.textpro("https://textpro.me/create-green-horror-style-text-effect-online-1036.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/textpro/magma', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.textpro("https://textpro.me/create-a-magma-hot-text-effect-online-1030.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/textpro/3d-neon-light', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.textpro("https://textpro.me/create-3d-neon-light-text-effect-online-1028.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/textpro/3d-orange-juice', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.textpro("https://textpro.me/create-a-3d-orange-juice-text-effect-online-1084.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/textpro/chocolate-cake', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.textpro("https://textpro.me/chocolate-cake-text-effect-890.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/textpro/strawberry', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.textpro("https://textpro.me/strawberry-text-effect-online-889.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/photooxy/flaming', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.photooxy("https://photooxy.com/logo-and-text-effects/realistic-flaming-text-effect-online-197.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/photooxy/shadow-sky', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.photooxy("https://photooxy.com/logo-and-text-effects/shadow-text-effect-in-the-sky-394.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/photooxy/metallic', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.photooxy("https://photooxy.com/other-design/create-metallic-text-glow-online-188.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/photooxy/naruto', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.photooxy("https://photooxy.com/manga-and-anime/make-naruto-banner-online-free-378.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/photooxy/pubg', cekKey, async (req, res, next) => {
	...
	var text2 = req.query.text2
	if (!text2 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text2"})  
	nekohime.photooxy("https://photooxy.com/battlegrounds/make-wallpaper-battlegrounds-logo-text-146.html", [text1,text2])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/photooxy/under-grass', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.photooxy("https://photooxy.com/logo-and-text-effects/make-quotes-under-grass-376.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/photooxy/harry-potter', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.photooxy("https://photooxy.com/logo-and-text-effects/create-harry-potter-text-on-horror-background-178.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/photooxy/flower-typography', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.photooxy("https://photooxy.com/art-effects/flower-typography-text-effect-164.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/photooxy/picture-of-love', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.photooxy("https://photooxy.com/logo-and-text-effects/create-a-picture-of-love-message-377.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/photooxy/coffee-cup', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.photooxy("https://photooxy.com/logo-and-text-effects/put-any-text-in-to-coffee-cup-371.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/photooxy/butterfly', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.photooxy("https://photooxy.com/logo-and-text-effects/butterfly-text-with-reflection-effect-183.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/photooxy/night-sky', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.photooxy("https://photooxy.com/logo-and-text-effects/write-stars-text-on-the-night-sky-200.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/photooxy/carved-wood', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.photooxy("https://photooxy.com/logo-and-text-effects/carved-wood-effect-online-171.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/photooxy/illuminated-metallic', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.photooxy("https://photooxy.com/logo-and-text-effects/illuminated-metallic-effect-177.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/photooxy/sweet-candy', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.photooxy("https://photooxy.com/logo-and-text-effects/sweet-andy-text-online-168.html", [text1])
.then((data) =>{ 
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/png'})
	res.send(data)
...
router.get('/api/soundoftext', cekKey, async (req, res, next) => {
	...
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	if (!lan ) return res.json({ status : false, creator : `${creator}`, message : "[!] silakan letak format lang dengan betul cek web site https://soundoftext.com/docs untuk lihat code lang"})   
...
router.get('/api/search/linkgroupwa', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
nekohime.linkwa(text1).then((data) =>{ 
...
router.get('/api/search/pinterest', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
nekohime.pinterest(text1).then((data) =>{ 
	if (!data[0] ) return res.json(loghandler.notfound)
	limitapikey(req.query.apikey)
    res.json({
	status: true,
...
router.get('/api/search/ringtone', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.ringtone(text1).then((data) =>{ 
...
router.get('/api/search/wikimedia', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
nekohime.wikimedia(text1).then((data) =>{ 
	if (!data[0] ) return res.json(loghandler.notfound)
	limitapikey(req.query.apikey)
    res.json({
	status: true,
...
router.get('/api/search/wallpaper', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.wallpaper(text1).then((data) =>{ 
	if (!data[0] ) return res.json(loghandler.notfound)
	limitapikey(req.query.apikey)
   res.json({
	status: true,
...
router.get('/api/search/sticker', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.stickersearch(text1).then(data => {
		if (!data ) return res.json(loghandler.notfound)
		limitapikey(req.query.apikey)
		res.json({
			status: true,
...
router.get('/api/search/sfilemobi', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})   
	nekohime.sfilemobiSearch(text1).then(data => {
		if (!data ) return res.json(loghandler.notfound)
		limitapikey(req.query.apikey)
		res.json({
			status: true,
...
router.get('/api/game/tembakgambar', cekKey, async (req, res, next) => {
 nekohime.tebakgambar().then((data) =>{ 
	limitapikey(req.query.apikey)	  
  res.json({
	status: true,
	creator: `${creator}`,
...
router.get('/api/nsfw/nsfwloli', cekKey, cekPrem, async (req, res, next) => {

	const Lol = JSON.parse(fs.readFileSync(__path +'/data/nsfwloli.json'));
  const randLol = Lol[Math.floor(Math.random() * Lol.length)];
  let data = await fetch(randLol).then(v => v.buffer());
   fs.writeFileSync(__path +'/tmp/lol.jpeg', data)
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/webp'})
	res.sendFile(__path+ '/tmp/lol.jpeg');
})

router.get('/api/nsfw/ahegao', cekKey, cekPrem, async (req, res, next) => {

	const ahe = JSON.parse(fs.readFileSync(__path +'/data/ahegao.json'));
  const randahe = ahe[Math.floor(Math.random() * ahe.length)];
 let data = await fetch(randahe).then(v => v.buffer());
   fs.writeFileSync(__path +'/tmp/ahe.jpeg', data)
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/webp'})
	res.sendFile(__path+ '/tmp/ahe.jpeg');
})

router.get('/api/nsfw/ass', cekKey, cekPrem, async (req, res, next) => {

	const ass = JSON.parse(fs.readFileSync(__path +'/data/ass.json'));
  const randass = ass[Math.floor(Math.random() * ass.length)];
 let data = await fetch(randass).then(v => v.buffer());
   fs.writeFileSync(__path +'/tmp/ass.jpeg', data)
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/webp'})
	res.sendFile(__path+ '/tmp/ass.jpeg');
})

router.get('/api/nsfw/bdsm', cekKey, cekPrem, async (req, res, next) => {

	const bdsm = JSON.parse(fs.readFileSync(__path +'/data/bdsm.json'));
  const randbdsm = bdsm[Math.floor(Math.random() * bdsm.length)];
  let data = await fetch(randbdsm).then(v => v.buffer());
   fs.writeFileSync(__path +'/tmp/bdsm.jpeg', data)
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/webp'})
	res.sendFile(__path+ '/tmp/bdsm.jpeg');
})

router.get('/api/nsfw/blowjob', cekKey, cekPrem, async (req, res, next) => {
  ...
  let data = await fetch(randblowjob).then(v => v.buffer());
   fs.writeFileSync(__path +'/tmp/blowjob.jpeg', data)
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/webp'})
	res.sendFile(__path+ '/tmp/blowjob.jpeg');
})

router.get('/api/nsfw/cuckold', cekKey, cekPrem, async (req, res, next) => {

	const cuckold = JSON.parse(fs.readFileSync(__path +'/data/cuckold.json'));
  const randblowjob = cuckold[Math.floor(Math.random() * cuckold.length)];
  let data = await fetch(randblowjob).then(v => v.buffer());
   fs.writeFileSync(__path +'/tmp/cuckold.jpeg', data)
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/webp'})
	res.sendFile(__path+ '/tmp/cuckold.jpeg');
})

router.get('/api/nsfw/cum', cekKey, cekPrem, async (req, res, next) => {

	const cum = JSON.parse(fs.readFileSync(__path +'/data/cum.json'));
  const randblowjob = cum[Math.floor(Math.random() * cum.length)];
  let data = await fetch(randblowjob).then(v => v.buffer());
   fs.writeFileSync(__path +'/tmp/cum.jpeg', data)
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/webp'})
	res.sendFile(__path+ '/tmp/cum.jpeg');
})

router.get('/api/nsfw/ero', cekKey, cekPrem, async (req, res, next) => {

	const ero = JSON.parse(fs.readFileSync(__path +'/data/ero.json'));
  const randblowjob = ero[Math.floor(Math.random() * ero.length)];
  let data = await fetch(randblowjob).then(v => v.buffer());
   fs.writeFileSync(__path +'/tmp/ero.jpeg', data)
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/webp'})
	res.sendFile(__path+ '/tmp/ero.jpeg');
})

router.get('/api/nsfw/femdom', cekKey, cekPrem, async (req, res, next) => {

	const femdom = JSON.parse(fs.readFileSync(__path +'/data/femdom.json'));
  const randblowjob = femdom[Math.floor(Math.random() * femdom.length)];
  let data = await fetch(randblowjob).then(v => v.buffer());
   fs.writeFileSync(__path +'/tmp/femdom.jpeg', data)
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/webp'})
	res.sendFile(__path+ '/tmp/femdom.jpeg');
})

router.get('/api/nsfw/foot', cekKey, cekPrem, async (req, res, next) => {

	const foot = JSON.parse(fs.readFileSync(__path +'/data/foot.json'));
  const randblowjob = foot[Math.floor(Math.random() * foot.length)];
  let data = await fetch(randblowjob).then(v => v.buffer());
   fs.writeFileSync(__path +'/tmp/foot.jpeg', data)
	limitapikey(req.query.apikey)
	res.set({'Content-Type': 'image/webp'})
...
router.get('/api/maker/dall-e', cekKey, cekPrem, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})  
	...
	const response = await openai.createImage({
	  ...
	  let dall = response.data.data[0].url
	  let data = await fetch(dall).then(v => v.buffer());
		fs.writeFileSync(__path +'/tmp/dalle.jpeg', data)
		limitapikey(req.query.apikey)
		res.set({'Content-Type': 'image/webp'})
		res.sendFile(__path+ '/tmp/dalle.jpeg');
})

router.get('/api/maker/toanime', cekKey, cekPrem, async (req, res) => {
	var url = req.query.url
	if (!url ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter url"})  
	h5tuqq(url).then(async (data) => {
		if (!data ) return res.json(loghandler.error) 
		limitapikey(req.query.apikey)
		res.json({
			status: true,
...
router.get('/api/maker/circle', cekKey, async (req, res) => {
	...
	if ( !img ) return res.json({ status : false, creator : 'Galang Purnama', message : "[!] cek kembali url image"}) 
...
router.get('/api/maker/darkness', cekKey, async (req, res) => {
	...
	var n = isNumber(no)
	if ( !img ) return res.json({ status : false, creator : 'Galang Purnama', message : "[!] cek kembali url image"}) 
	if ( !n ) return res.json({ status : false, creator : 'Galang Purnama', message : "[!] parameter no nombor sahaja"}) 
...
router.get('/api/maker/resize', cekKey, async (req, res) => {
	...
	if ( !img ) return res.json({ status : false, creator : 'Galang Purnama', message : "[!] cek kembali url image"}) 
	if ( !wid ) return res.json({ status : false, creator : 'Galang Purnama', message : "[!] parameter width nombor sahaja"}) 
	if ( !hei ) return res.json({ status : false, creator : 'Galang Purnama', message : "[!] parameter height nombor sahaja"}) 
...
router.get('/api/maker/welcome1', cekKey, async (req, res, next) => {
    ...
    if (!name ) return res.json({ status : false, creator : 'Galang Purnama', message : "[!] masukan parameter name"})  
	if (!grup ) return res.json({ status : false, creator : 'Galang Purnama', message : "[!] masukan parameter gpname"})  
    if (!member ) return res.json({ status : false, creator : 'Galang Purnama', message : "[!] masukan parameter member"})  
	if (!pp ) return res.json({ status : false, creator : 'Galang Purnama', message : "[!] masukan parameter pp"})  
    if (!bg ) return res.json({ status : false, creator : 'Galang Purnama', message : "[!] masukan parameter bg"})  

	if ( !imgpp ) return res.json({ status : false, creator : 'Galang Purnama', message : "[!] cek kembali url image pp"}) 
	if ( !bgimg ) return res.json({ status : false, creator : 'Galang Purnama', message : "[!] cek kembali url image bg"}) 
	Canvas.registerFont('./asset/font/theboldfont.ttf', { family: 'Bold' })

var welcomeCanvas = {}
welcomeCanvas.create = Canvas.createCanvas(1024, 510)
welcomeCanvas.context = welcomeCanvas.create.getContext('2d')
welcomeCanvas.context.font = '72px Bold'
welcomeCanvas.context.fillStyle = '#ffffff'

await Canvas.loadImage("./asset/image/wbg1.jpg").then(async (img) => {
    welcomeCanvas.context.drawImage(img, 0, 0, 1024, 510)
...
.then(bg => {
can.context.drawImage(bg, 0, 0, 1024, 510)
})

    let canvas = welcomeCanvas
	canvas.context.font = '80px Bold',
    canvas.context.textAlign = 'center'
    canvas.context.fillText(name.toUpperCase(), 512, 388)
    canvas.context.font = '45px Bold'
	canvas.context.textAlign = 'center'
	canvas.context.fillText(`Welcome To ${grup}`.toUpperCase(), 512, 443)
	canvas.context.font = '25px Bold'
	canvas.context.fillText(`Jumlah Member: ${member}th`, 512, 478)
...
router.get('/api/maker/goodbye1', cekKey, async (req, res, next) => {
    ...
    if (!name ) return res.json({ status : false, creator : 'nekohime', message : "[!] masukan parameter name"})  
	if (!grup ) return res.json({ status : false, creator : 'nekohime', message : "[!] masukan parameter gpname"})  
    if (!member ) return res.json({ status : false, creator : 'nekohime', message : "[!] masukan parameter member"})  
    if (!bg ) return res.json({ status : false, creator : 'nekohime', message : "[!] masukan parameter bg"})  
	if (!pp) return res.json({ status : false, creator : 'nekohime', message : "[!] masukan parameter pp"}) 
   
	if ( !imgpp ) return res.json({ status : false, creator : 'nekohime', message : "[!] masukan parameter pp Link pp dengan betul"}) 
	if ( !bgimg ) return res.json({ status : false, creator : 'nekohime', message : "[!] masukan parameter pp Link bg dengan betul"}) 

	Canvas.registerFont('./asset/font/theboldfont.ttf', { family: 'Bold' })

	var GoodbayCanvas = {}
	GoodbayCanvas.create = Canvas.createCanvas(1024, 510)
	GoodbayCanvas.context = GoodbayCanvas.create.getContext('2d')
	GoodbayCanvas.context.font = '72px Bold'
	GoodbayCanvas.context.fillStyle = '#ffffff'
	
	await Canvas.loadImage("./asset/image/wbg1.jpg").then(async (img) => {
		GoodbayCanvas.context.drawImage(img, 0, 0, 1024, 510)
		GoodbayCanvas.context.beginPath()
		GoodbayCanvas.context.arc(512, 166, 128, 0, Math.PI * 2, true)
		GoodbayCanvas.context.stroke()
		GoodbayCanvas.context.fill()
	})
	
	let can = GoodbayCanvas
	...
	can.context.drawImage(bg, 0, 0, 1024, 510)
		...
		let canvas = GoodbayCanvas
		canvas.context.font = '80px Bold',
		canvas.context.textAlign = 'center'
		canvas.context.fillText(name.toUpperCase(), 512, 388)
		canvas.context.font = '45px Bold'
		canvas.context.textAlign = 'center'
		canvas.context.fillText(`Goodbay To ${grup}`.toUpperCase(), 512, 443)
		canvas.context.font = '25px Bold'
		canvas.context.fillText(`Jumlah Member: ${member}th`, 512, 478)
		canvas.context.beginPath()
		canvas.context.arc(512, 166, 119, 0, Math.PI * 2, true)
...
router.get('/api/tools/openai', async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})  
	if (text1.length > 2048) return res.json({ status : false, creator : `${creator}`, message : "[!] Maximal 2.048 String!"})
...
router.get('/api/islamic/surah', cekKey, async (req, res, next) => {
	...
	nekohime.surah(text1).then((data) =>{ 
	if (!data ) return res.json(loghandler.notfound)
	limitapikey(req.query.apikey)
		res.json({
			status: true,
...
router.get('/api/islamic/tafsirsurah', cekKey, async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter text"})  
	nekohime.tafsirsurah(text1).then((data) =>{ 
	if (!data[0] ) return res.json(loghandler.notfound)
	limitapikey(req.query.apikey)
		res.json({
			status: true,
